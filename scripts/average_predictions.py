...
for message in consumer1:
    ...
    if topic == 'result1':
        results[0, :] = np.concatenate((feature, np.reshape([float(iter)], (1, 1))), axis=1)
    elif topic == 'result2':
        results[1, :] = np.concatenate((feature, np.reshape([float(iter)], (1, 1))), axis=1)
    elif topic == 'result3':
        results[2, :] = np.concatenate((feature, np.reshape([float(iter)], (1, 1))), axis=1)
    else:
        results[3, :] = np.concatenate((feature, np.reshape([float(iter)], (1, 1))), axis=1)
